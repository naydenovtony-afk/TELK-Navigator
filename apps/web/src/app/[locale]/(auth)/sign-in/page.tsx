import { signIn } from '@/auth'
import { getTranslations } from 'next-intl/server'

export async function generateMetadata() {
  const t = await getTranslations('telk.auth.login')
  return { title: t('title') }
}

export default async function SignInPage() {
  const t = await getTranslations('telk.auth.login')

  return (
    <div className="min-h-screen flex items-center justify-center bg-medical-surface px-4">
      <div className="w-full max-w-md">
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="font-display text-5xl text-medical-navy mb-2">
            ТЕЛК Навигатор
          </h1>
          <p className="text-medical-slate text-sm">
            Навигирайте процеса на ТЕЛК с увереност
          </p>
        </div>

        {/* Card */}
        <div className="bg-white rounded-2xl border border-medical-border shadow-sm p-8">
          <h2 className="font-display text-2xl text-medical-navy mb-1">
            {t('title')}
          </h2>
          <p className="text-sm text-medical-slate mb-6">
            Използвайте вашия Google акаунт за бърз и сигурен вход.
          </p>

          <form
            action={async () => {
              'use server'
              await signIn('google', { redirectTo: '/bg/dashboard' })
            }}
          >
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-3 bg-white border border-medical-border text-dark-text font-medium py-3 px-4 rounded-xl hover:bg-medical-surface transition-colors text-sm"
            >
              <GoogleIcon />
              Вход с Google
            </button>
          </form>

          <p className="text-xs text-medical-slate text-center mt-6">
            С влизането си приемате условията за обработка на лични данни
            съгласно GDPR.
          </p>
        </div>
      </div>
    </div>
  )
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
    </svg>
  )
}
