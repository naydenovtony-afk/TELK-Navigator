...
export async function PATCH(req: NextRequest) {
  ...
  const pw = await db.query.userPasswords.findFirst({ where: eq(userPasswords.userId, userId) })
  if (!pw) {
    return NextResponse.json({ error: 'Акаунтът е свързан с Google/OAuth и няма локална парола.' }, { status: 400 })
  }
  ...
  await db.update(userPasswords).set({ hash: newHash }).where(eq(userPasswords.userId, userId))
